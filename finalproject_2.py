...
plt.title('Unit price vs total (Test set)')
plt.xlabel('Unit price')
plt.ylabel('total')
plt.show()
...
""""(Q15 CLO3)	Predict the value of the dependent variable from the different classifier such as Logistic Regression, KNN, Naïve-Bayes and Decision Tree."
"""
...
# Identify the best-fit model based on F1-Score
best_model = metrics_df.loc[metrics_df['F1-Score'].idxmax()]
print("\nBest-Fit Model:")
print(f"Model: {best_model['Model']}")
print(f"Reasons: Highest F1-Score ({best_model['F1-Score']:.2f}), Precision ({best_model['Precision']:.2f}), and Recall ({best_model['Recall']:.2f})")
...
X = dataset[['Total', 'gross income', 'Rating']].values
...
plt.title('Clustering (Total vs Rating)')
plt.xlabel('Branch (Encoded)')
plt.ylabel('Rating')
plt.legend()
